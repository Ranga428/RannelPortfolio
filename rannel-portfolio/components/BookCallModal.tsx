...
export default function BookCallModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus({ type: 'success', message: 'Your inquiry has been sent! I will get back to you within 24 hours.' });
      setTimeout(() => {
        handleClose();
        setEmail("");
        setAvailableTime("");
      ...
      <div className="relative z-51 bg-[var(--color-bg-main)] rounded-none max-w-lg w-full p-6 md:p-8 border border-[var(--color-border)]">
        ...
        {status ? (
          <div className={`p-6 text-center ${status.type === 'success' ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20'}`}>
            ...
            <p className={`font-sans ${status.type === 'success' ? 'text-green-800 dark:text-green-800 dark:text-green-200' : 'text-red-800 dark:text-red-200'}`}>
              {status.message}
            </p>
          </div>
        ) : (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <DatePicker
                    ref={datePickerRef}
                    selected={availableTime ? new Date(availableTime) : null}
                    onChange={(date: Date | null) => {
                      if (date) {
                        setAvailableTime(date.toISOString().slice(0, 16));
                      } else {
                        setAvailableTime("");
                      }
                    ...
                    customInput={
                      <input
                        className="w-full px-4 py-2.5 border border-[var(--color-border)] bg-[var(--color-bg-main)] text-[var(--color-text-heading)] rounded-none focus:outline-none focus:border-[#B84A39] h-[42px]"
                  ...
                  <button
                    ...
                    className="absolute right-3 top-[50%] -translate-y-1/2 text-[var(--color-text-body)] hover:text-[#B84A39] z-10 cursor-pointer"
                  >
                    {datePickerOpen || availableTime ? (
          ...
          <div className="pt-2 flex flex-col items-center">
            ...
            <p className="text-xs text-[var(--color-text-body)] mb-3 text-left w-full">
